...
class ContourPlot():
    ...
    def __init__(self, file_path):
        ...
        self.df = pd.read_csv(file_path, sep=None, engine="python", on_bad_lines="skip")
        ...
        if not self.df.empty:
            shift_col = self.df.columns[0]
            self.df = self.df.rename(columns={shift_col: "chemical_shift_ppm"})
    ...
    def plot(self, zmin, zmax):
        ...
        # ----- 1) CASE: self.Z is empty → show fallback figure -----
        if Z.size == 0 or Z.shape[0] == 0 or Z.shape[1] == 0:
            fig, ax = plt.subplots(figsize=(8, 3))
            ax.text(
                0.5, 0.5,
                "No contour data available.\n(This dataset may have no time dimension.)",
        ...
        Zmax = Z.max()
        if Zmax <= 0:
            Zmax = 1.0  # Prevent zero-division issues
        # änderung ende
