...
class Panel1SpectrumPlot():
    ...
    def __init__(self, file_path):
        ...
        self.max_points = 5000  # cap points sent to browser

        # Ensure the plot directory exists 
        os.makedirs(self.plot_dir, exist_ok=True)
        
        # robust loaders: delimiter sniffing, decimal commas -> dots, numeric coercion, drop NaNs/infs
        def _load_clean_csv(fp):
            ...
            if not df.empty:
                shift_col = df.columns[0]
                df = df.rename(columns={shift_col: "chemical_shift_ppm"})
            return df

        self.data = _load_clean_csv(file_path)
        self.sum_data = _load_clean_csv(Path('output', f'{self.file_name}_output', 'sum_fit.csv'))
        self.differences = _load_clean_csv(Path('output', f'{self.file_name}_output', 'differences.csv'))
        ...
        for f in ind_file_names:
            fp = Path('output', f'{self.file_name}_output', 'substance_fits', f)
            self.individual_fits.append(_load_clean_csv(fp))

    def _downsample_xy(self, x_series, y_series):
        """Reduce number of points to avoid oversized browser payloads."""
        n = len(x_series)
    ...
    def plot_raw(self, frame):
        ...
        # Add Data to Fig
        fig = go.Figure()
        x_raw = self.data.iloc[:,0][::-1]
        y_raw = self.data.iloc[:,frame][::-1]
        x_raw, y_raw = self._downsample_xy(x_raw, y_raw)
        fig.add_trace(go.Scatter(x=x_raw,       # To Change direction of x axis from low to high 
                                 y=y_raw,       # To Change direction of x axis from low to high 
                                 mode='lines', name='Raw',
    ...
    def plot_diff(self, frame):
        ...
        # Add data to Fig
        fig = go.Figure()
        x_diff = self.differences.iloc[:,0][::-1]
        y_diff = self.differences.iloc[:,frame][::-1]
        x_diff, y_diff = self._downsample_xy(x_diff, y_diff)
        fig.add_trace(go.Scatter(x=x_diff,        # To Change direction of x axis from low to high 
                                 y=y_diff,    # To Change direction of x axis from low to high 
    ...
    def plot_sum_fit(self, frame):
        ...
        x_sum = self.sum_data.iloc[:,0][::-1]
        y_sum = self.sum_data.iloc[:,frame][::-1]
        x_sum, y_sum = self._downsample_xy(x_sum, y_sum)
        fig.add_trace(go.Scatter(x=x_sum,           # To Change direction of x axis from low to high 
                                 y=y_sum,       # To Change direction of x axis from low to high 
                                 mode='lines', 
                                 name='Sum Fit',
        ...
        for i in range(1, len(frame_data.columns)):
            ...
            x_ind = self.sum_data.iloc[:,0]
            y_ind = frame_data.iloc[:,i]
            x_ind, y_ind = self._downsample_xy(x_ind, y_ind)
            fig.add_trace(go.Scatter(x=x_ind, 
                                     y=y_ind, 
                                     mode='lines', 
